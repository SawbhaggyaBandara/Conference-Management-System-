...
$sql_create_session_table = "
    CREATE TABLE IF NOT EXISTS users ( 
        Session id (Primary Key, INT, Auto Increment)
        name (VARCHAR)
        Speaker (VARCHAR)
        Capacity (INT)
        Registered count (INT)
        Time and location (VARCHAR)
    );
";


$sql_create_participant_table = "
    CREATE TABLE IF NOT EXISTS users ( 
        User id (Primary Key, INT from users table)
        name (VARCHAR)
        email (VARCHAR)
        Organization (VARCHAR)
        Session registered (Foreign Key, INT from sessions table)
        Qr code path (VARCHAR)
